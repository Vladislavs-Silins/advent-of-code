/**
Advent of Code[About][Events][Shop][Settings][Log Out]Vlad Silin 3*
...
const replaceSymbol = (
  ...
  const letteredWord = word.split("");
  letteredWord[position] = symbol;
  return letteredWord.join("");
};
...
export const solution = (): number => {
  ...
  for (const checkedPassword of input) {
    ...
    const minReplacePassword = replaceSymbol(password, letter, min - 1);
    const maxReplacePassword = replaceSymbol(password, letter, max - 1);
    const isMinPosition = password === minReplacePassword;
    const isMaxPosition = password === maxReplacePassword;
    if (!(isMaxPosition && isMinPosition) && (isMaxPosition || isMinPosition)) {
      count++;
