...
import { Command, Sign, Simulation } from "./types";
...
describe("puzzle", () => {
  describe("solution()", () => {
    ...
    it("should return right answer", async () => {
      mockPuzzleTransform.mockRestore();
      const result = solution();
      const expectedResult = 797;
      expect(result).toEqual(expectedResult);
    });
  });
  describe("hasInfiniteLoop()", () => {
    it("should return true for infinite test set", async () => {
      mockPuzzleTransform.mockReturnValue(MOCKED_INPUT);
      const instructions = getInstructions();
      const result = hasInfiniteLoop(instructions);
      expect(result).toBe(true);
    });
    it("should return true for successful test set", async () => {
  ...
  describe("fixOneInfiniteLoop()", () => {
    it("should return list of instructions", async () => {
      mockPuzzleTransform.mockReturnValue(MOCKED_INPUT);
      const instructions = getInstructions();
      const fixedInstructions = fixOneInfiniteLoop(instructions);
      expect(Array.isArray(fixedInstructions)).toBe(true);
      expect(isInstruction(fixedInstructions[0])).toBe(true);
    });
    it("should return list of 9 instructions", async () => {
      mockPuzzleTransform.mockReturnValue(MOCKED_INPUT);
      const instructions = getInstructions();
      const fixedInstructions = fixOneInfiniteLoop(instructions);
      expect(fixedInstructions.length).toBe(9);
    });
    it("should change correct instruction", async () => {
      mockPuzzleTransform.mockReturnValue(MOCKED_INPUT);
      const instructions = getInstructions();
      const fixedInstructions = fixOneInfiniteLoop(instructions);
      const changedInstruction = fixedInstructions[7];
      expect(changedInstruction).toEqual({
        command: Command.nop,
        sign: Sign.minus,
        value: 4,
      });
